...
def _put_trans_data(transCmd, parameters, data, noPad=False):
	...
	if len(data):
		...
		transData3 = bytetransData.decode()
		if isinstance(data, str):
			transData = transData3 + data
			
		else:
			str_data = data.decode('iso-8859-1')
			transData = transData3 + str_data
...
class MYSMB(smb.SMB):
	...
	def create_smb_packet(self, smbReq, mid=None, pid=None, tid=None):
		...
		req4 = req3.decode()
		req5 = pkt.__str__()
		req6 = req5.decode('iso-8859-1')
		req = req4+req6
		return req

	def send_raw(self, data):
		fixed_data = data.encode('iso-8859-1')
		data = fixed_data
